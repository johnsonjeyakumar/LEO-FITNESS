...
import { Mail, Lock, Loader2, Eye, EyeOff, Dumbbell } from 'lucide-react';
...
const Login: React.FC<Props> = ({ setAuthView }) => {
  const { login } = useAuth();
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-black relative overflow-hidden font-sans">
      ...
      >
        ...
        <div className="text-center mb-8 relative">
          ...
          <h2 className="text-3xl font-display font-bold text-white uppercase tracking-tight">Initiate Protocol</h2>
          <p className="text-gray-400 text-sm mt-1">Access your personalized LEO AI interface</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                disabled={loading}
                className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-5 py-4 focus:border-primary/50 focus:bg-white/10 focus:ring-1 focus:ring-primary/50 outline-none text-white placeholder-gray-600 transition-all text-sm disabled:opacity-55"
              />
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                placeholder="••••••••"
                disabled={loading}
                className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-12 py-4 focus:border-primary/50 focus:bg-white/10 focus:ring-1 focus:ring-primary/50 outline-none text-white placeholder-gray-600 transition-all text-sm disabled:opacity-55"
              />
          ...
          <motion.button
            ...
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-primary to-orange-600 text-black font-display font-bold text-lg uppercase py-4 rounded-xl hover:shadow-lg hover:shadow-orange-900/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <>
