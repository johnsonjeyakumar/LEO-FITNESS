...
const ForgotPassword: React.FC<Props> = ({ setAuthView }) => {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-black relative overflow-hidden font-sans">
      ...
      >
        ...
        <div className="text-center mb-8 relative">
          ...
          <h2 className="text-3xl font-display font-bold text-white uppercase tracking-tight">Reset Password</h2>
          <p className="text-gray-400 text-sm mt-1">Receive password override links via secure email</p>
        </div>
        ...
        {message && (
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="p-4 mb-6 bg-green-500/10 border border-green-500/20 text-green-400 rounded-xl text-xs font-semibold"
          >
            {message}
          </motion.div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="text-xs text-primary font-bold uppercase tracking-wider block">Registered Email</label>
